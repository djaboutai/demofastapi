...
@router.get("/", tags=["users"], summary="Returns all usernames.")
async def get_all_users():
    response = jsonable_encoder([{"username": "Rick"}, {"username": "Morty"}])
    return JSONResponse(status_code=status.HTTP_200_OK, content=response)


@router.get("/me", tags=["users"], summary="Returns logged username.")
async def get_user_me():
    response = jsonable_encoder({"username": "fakecurrentuser"})
    return JSONResponse(status_code=status.HTTP_200_OK, content=response)


@router.get("/{username}", tags=["users"], summary="Returns desired username.")
